"""ViT (Vision Transformer) + CIFAR-10 실행 시간 측정 실험

ViT는 이미지를 패치로 나눠 Transformer Encoder에 입력하는 표준 Vision Transformer.
timm 라이브러리에서 Small / Base / Large 3가지 변형을 가져와 실험.

실험 설정:
    - 모델    : vit_small / vit_base / vit_large (patch16)
...
사용법:
    python collect/vit_collector.py
"""
...
from models.vit import create_vit_variants
...
OUTPUT_PATH = os.path.join(ROOT_DIR, 'data', 'vit_results.csv')
    ...
    for device_str in devices:
        ...
        for model_tmpl, config in variants:
            ...
            print(f"[{count}/{total}] {model_name}, embed={embed_dim}, "
                  f"layers={num_layers}, heads={num_heads}, device={device_str}, "
