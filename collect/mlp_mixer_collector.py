"""MLP-Mixer + CIFAR-10 실행 시간 측정 실험

MLP-Mixer는 Attention 없이 MLP만으로 이미지를 처리하는 모델.
timm 라이브러리에서 Small / Base / Large 3가지 변형을 가져와 실험.

실험 설정:
    - 모델    : mixer_s16_224 / mixer_b16_224 / mixer_l16_224
...
사용법:
    python collect/mlp_mixer_collector.py
"""
...
from models.mlp_mixer import create_mlp_mixer_variants
...
# MLP-Mixer는 224×224 입력을 기준으로 설계됨
INPUT_CONFIG = {
...
OUTPUT_PATH = os.path.join(ROOT_DIR, 'data', 'mlp_mixer_results.csv')
...
def run():
    print("=" * 60)
    print("  MLP-Mixer + MNIST 실행 시간 측정")
    print("=" * 60)

    variants = create_mlp_mixer_variants()
    devices  = get_devices()
    ...
    for device_str in devices:
        ...
        for model_tmpl, config in variants:
            ...
            # 매 측정마다 새 모델 생성 (fresh 모델로 측정)
            def model_fn(mn=model_name):
                import timm
                return timm.create_model(mn, pretrained=False, num_classes=10, in_chans=1)
            ...
            print(f"[{count}/{total}] {model_name}, layers={num_layers}, "
                  f"hidden={hidden_dim}, patch={patch_size}, device={device_str}, "
