...
사용법:
    python scripts/run_benchmark.py                   # 전체 실행
    python scripts/run_benchmark.py --model simple_ann
    python scripts/run_benchmark.py --model transformer
    python scripts/run_benchmark.py --model gan
    python scripts/run_benchmark.py --device cpu
    python scripts/run_benchmark.py --repeats 3
    python scripts/run_benchmark.py --resume          # 중단 후 이어서 실행
    python scripts/run_benchmark.py --adaptive-epochs --max-epochs 10 --target-accuracy 65
"""
import argparse
import os
import sys
...
def parse_args():
    ...
    parser.add_argument('--adaptive-epochs', action='store_true',
                        help='분류 모델만: 정확도 목표에 도달하거나 --max-epochs까지 에폭 추가 '
                             '(학습 시간은 누적 측정)')
    parser.add_argument('--max-epochs', type=int, default=10,
                        help='--adaptive-epochs 시 상한 에폭 수 (기본: 10)')
    parser.add_argument('--target-accuracy', type=float, default=65.0,
                        help='--adaptive-epochs 시 이 테스트 정확도(%%) 이상이면 조기 종료')
...
def main():
    ...
    runner = ExperimentRunner(
        ...
        adaptive_training=args.adaptive_epochs,
        max_adaptive_epochs=args.max_epochs,
        target_accuracy=args.target_accuracy,
    )
    if args.adaptive_epochs:
        print(
            f"적응형 에폭 모드: 목표 정확도 {args.target_accuracy}% 또는 "
            f"최대 {args.max_epochs} 에폭까지 학습 후 측정합니다.\n"
        )
