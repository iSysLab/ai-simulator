# CPU 메타모델로 results/onnx_samples 전부 예측 (프로젝트 루트에서 실행)
# 사용: powershell -ExecutionPolicy Bypass -File scripts/run_onnx_predictions_cpu.ps1
...
$common = @(
    "--device", "cpu",
    "--model-dir", "results/trained_models_cpu"
)
...
Write-Host "`nDone: all ONNX samples (CPU meta-models, cpu)." -ForegroundColor Green
