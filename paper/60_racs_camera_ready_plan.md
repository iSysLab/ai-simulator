...
**남은 결정(월요일)**: 데스크톱 CPU를 재측정할지다. MPS가 깨끗하므로 맥 쪽은 v2로 통일 가능하고,
Mac CPU만 v1으로 남는다. 데스크톱은 CUDA v1과 v2가 다른 기기 상태이므로 둘 중 하나를 골라야 한다.
권장 순서는 (1) 전원 옵션 "고성능"·NVIDIA "최대 성능 선호"·게임 런처 종료 후 소형 3구성 프로브(5분),
(2) 3월 값이 재현되면 CUDA만 그 상태에서 2.4h 재측정하고 CPU는 v1 유지, (3) 재현되지 않으면
데스크톱 CPU 27h 재측정(주말)으로 데스크톱 두 백엔드를 한 세션에 맞춘다.
